...
public class MatrizMath {
	...
	public MatrizMath trasponerMatriz(){
		MatrizMath transpuesta = new MatrizMath(columna,fila);
		for( int i=0 ; i<fila ; i++)
			for( int j=0 ; j<columna ; j++)
				transpuesta.coord[j][i] = coord[i][j];
		return transpuesta;
	}
	
	//Terminar funcion 
	public MatrizMath invertirMatriz(){
		MatrizMath matrizInvertida = new MatrizMath(fila,columna);
